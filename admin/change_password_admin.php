...
if ($stmt) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        header('Location: view_admin.php?status=not_found');
        exit;
    }
    $stmt->close();
} else {
    ...
    if (strlen($new_password) < 6) {
        ...
        if ($update_stmt) {
            ...
            if ($update_stmt->execute()) {
                header("Location: view_admin.php?id=" . urlencode($user_id) . "&status=password_changed&nav_drop=true");
                exit;
            } else {
        ...
        <div class="col-12 col-md-6">
          <div class="p-4">
            ...
            <form method="post" class="mt-3">
              ...
              <a href="view_admin.php?id=<?= urlencode($user_id) ?>&nav_drop=true" class="btn btn-secondary ms-2">Cancel</a>
            </form>
