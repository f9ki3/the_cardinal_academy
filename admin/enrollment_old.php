...
// Count total results for pagination (Only counts records ready for enrollment)
$count_query = "SELECT COUNT(*) as total 
                FROM admission_old 
                WHERE admission_status = 'approved' AND (
                    que_code LIKE '%$search_safe%' 
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    rows.forEach(row => {
      row.addEventListener('click', (event) => {
        ...
        // Check if the click target is the 'Enroll' button (or its parent link)
        if (event.target.closest('a') && event.target.closest('a').classList.contains('btn-primary')) {
             // Let the anchor link handle the navigation (to enroll_action_old.php)
             return; 
        }

        // Navigate to view_enrollement_old.php if the click is not on the button
