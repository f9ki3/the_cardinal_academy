...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($student_number)) {
    ...
    $disciplinary_incharge = trim($_POST['disciplinary_incharge'] ?? '');
    ...
    $stmt = $conn->prepare("
        ...
        (disciplinary_id, student_id, incident_date, incident_location, incident_description, violation_type, disciplinary_action, witnesses,disciplinary_incharge, remarks, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    $stmt->bind_param(
        "ssssssssss",
