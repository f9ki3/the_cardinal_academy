...
// ✅ Base SQL for disciplinary records
        ...
        <div class="row align-items-center mb-3">
          ...
          <div class="col-md-6">
            <form method="get" class="d-flex">
              <input type="text" name="search" class="form-control me-2" placeholder="Search by Disciplinary ID or Student Number" value="<?= htmlspecialchars($search) ?>">
              ...
              <?php if ($records->num_rows > 0): ?>
                <?php while ($row = $records->fetch_assoc()): ?>
                  <tr class="clickable-row" onclick="window.location='view_disciplinary_detail.php?disciplinary_id=<?= urlencode($row['disciplinary_id']) ?>&student_id=<?= urlencode($row['student_id']) ?>'">
                    <td class="text-muted py-3"><?= htmlspecialchars($row['incident_date']) ?></td>
                ...
                <?php endwhile; ?>
              <?php else: ?>
                <tr><td colspan="9" class="text-center text-muted">No disciplinary records found.</td></tr>
              <?php endif; ?>
            </tbody>
